...
public:
    ...
    {
        // 订阅当前激光帧运动畸变校正后的点云信息
        subLaserCloudInfo = nh.subscribe<my_slam_liosam::cloud_info>("lio_sam/deskew/cloud_info", 1, &FeatureExtraction::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
        
        // 发布当前激光帧提取特征之后的点云信息
        pubLaserCloudInfo = nh.advertise<my_slam_liosam::cloud_info>("lio_sam/feature/cloud_info", 1);
        // 发布当前激光帧的角点点云
    ...
    void calculateSmoothness()
    {
        ...
        {
            // 用当前激光点前后5个点计算当前点的曲率，平坦位置处曲率较小，角点处曲率较大；这个方法简单有效
    ...
    void markOccludedPoints()
    {
        ...
        for (int i = 5; i < cloudSize - 6; ++i)
        {
            ...
            // TODO:::
            //  如果两个激光点之间的一维索引差值，如果在同一条扫描线上，那么值为1；如果两个点之间有一些无效点被剔除了，可能会比1大，但不会特别大
