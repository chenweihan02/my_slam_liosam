#include "utility.h"
#include "my_slam_liosam/cloud_info.h"
...
private:
    ...
    // 用于存储每个点的距离信息，用于点云的去畸变和投影
    // 在projectPointCloud中，根据点的实际位置计算出相应的列行坐标
    // 在cloudExtraction中，用于统计有效点的数量，只取其上有效的点
    ...
    my_slam_liosam::cloud_info cloudInfo;
    // 当前帧起始时刻 // 雷达帧扫描开始时间戳
    double timeScanCur;
    // 当前帧结束时刻 // 雷达帧扫描结束时间戳
    double timeScanEnd;
...
public:
    ...
    */
    ...
    deskewFlag(0)
    {
        ...
        // 发布当前激光帧运动畸变矫正后的点云信息
        pubLaserCloudInfo = nh.advertise<my_slam_liosam::cloud_info> ("lio_sam/deskew/cloud_info", 1);
    ...
    */
     ...
     *  imu数据:
     *      1) 遍历当前激光帧起止时刻之间的imu数据，初始时刻对应的imu的姿态角RPY设为当前帧的初始姿态角
     *      2) 用角速度、时间积分，计算每一时刻相对于初始时刻的旋转量，初始时刻旋转设为0
     *  imu里程计数据:
     *      1) 遍历当前激光帧起止时刻之间的imu里程计数据，初始时刻对应imu里程计
    ...
    */
    // TODO 预处理数据，虽然还是没看懂 这个数据的用途
    void imuDeskewInfo()
    {
        cloudInfo.imuAvailable = false;

        // 从imu队列中删除当前激光帧0.01s前面时刻的imu数据
        // TOOD：为啥要删除，而且时间为啥是0.01
        /**
         * 在lio-sam中，imu数据的时间戳是比激光数据的时间戳更早，因此需要将imu的数据时间戳与激光数据的时间戳对齐
         * 而由于imu的数据频率比激光数据高，所以需要删除一部分imu数据，以保证imu数据和激光数据之间的时间差小于一个阈值， 这个阈值在lio-sam中默认为0.01s
        */
    ...
    */
    void odomDeskewInfo()
    {
        ...
        // TODO: imudeskewinfo, odomdeskewinfo 的tf 计算的原理
        // 提取imu里程计姿态角
        tf::Quaternion orientation;
    ...
    */
    ...
    {
        // 如果传感器移到哦你速度较慢，例如人行走的速度，那么可以认为激光在一帧时间范围内，平移量小到可以忽略不计
    ...
    /**
     * 激光运动畸变校正
     * 利用当前帧起止时刻之间的imu数据计算旋转增量，imu里程计数据计算平移增量，进而将每一时刻激光点位置变换到第一个激光坐标系下，进行运动补偿
    */
    // TODO???
